...
interface StoreCatalogueProps {
  ...
  dynamicFilterOptions?: boolean;
}

const DEFAULT_TITLE = 'Sustainable Corporate Gifts & Keepsakes';
...
const UMBRELLA_CATEGORY_SLUGS = ['personalised-woodwork'];
const WOODWORK_CATEGORY_ID = 'cat-personalised-woodwork';
const WOODWORK_NAME = 'personalised woodwork';
...
export default function StoreCatalogue({
  ...
  // Products scoped to this page: the dedicated woodwork page shows only that
  // group; the main moss page hides it entirely (it is not a catalogue filter).
  const scopedProducts = useMemo(() => {
    const isWoodworkScope = hideCategoryFilter && initialCategorySlug === 'personalised-woodwork';
    return products.filter((p) => {
      const isWoodwork =
        p.category_id === WOODWORK_CATEGORY_ID ||
        (p.category_name && p.category_name.toLowerCase() === WOODWORK_NAME);
      return isWoodworkScope ? isWoodwork : !isWoodwork;
    });
  }, [products, hideCategoryFilter, initialCategorySlug]);

  // Derive unique materials from scoped products
  ...
  const tiersList = useMemo(() => {
    if (!dynamicFilterOptions) return ['Essential', 'Premium', 'Signature', 'Luxury'];
    const set = new Set<string>(scopedProducts.map((p) => p.tier).filter(Boolean));
    return Array.from(set);
  }, [scopedProducts, dynamicFilterOptions]);

  const speedList = useMemo(() => {
    if (!dynamicFilterOptions) return ['Ready to Ship', '3-5 Days', '7-10 Days'];
    const set = new Set<string>(scopedProducts.map((p) => p.speed).filter(Boolean));
    return Array.from(set);
  }, [scopedProducts, dynamicFilterOptions]);

  // Filtered & Sorted scoped products
  const filteredProducts = useMemo(() => {
    ...
    return list;
  }, [scopedProducts, categories, searchQuery, selectedCategory, selectedMaterial, selectedTier, selectedSpeed, selectedCollection, priceRange, sortBy]);

  const activeFiltersCount = useMemo(() => {
    let count = 0;
    if (selectedCategory !== 'all' && !hideCategoryFilter) count++;
            ...
            <div>
              ...
              <div className="flex flex-col gap-0.5">
                ...
                >
                  ...
                  <span className="text-[10px] text-stone-400 font-medium">{scopedProducts.length}</span>
                </button>
                {categories
                  .filter((cat) => !UMBRELLA_CATEGORY_SLUGS.includes(cat.slug))
                  .map((cat) => {
              ...
              <div className="flex flex-wrap items-center gap-2 bg-white p-3.5 rounded-2xl border border-[#EBE4D8]">
                ...
                {!hideCategoryFilter && selectedCategory !== 'all' && (
                  <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-[#FAF8F5] border border-[#DCD1C4] text-stone-700">
                    ...
                    <X className="w-3 h-3 cursor-pointer" onClick={() => setSelectedCategory(initialCategorySlug || 'all')} />
                  </span>
                )}
                {selectedMaterial !== 'all' && (
                  <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg bg-[#FAF8F5] border border-[#DCD1C4] text-stone-700">
            ...
            <div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {categories
                    .filter((c) => !UMBRELLA_CATEGORY_SLUGS.includes(c.slug))
                    .map((c) => (
