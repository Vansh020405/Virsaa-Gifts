...
export default function Navbar() {
  ...
  const isCatalogueActive = pathname === '/catalogue' || pathname === '/woodwork';
  ...
  useEffect(() => {
    setDesktopCatalogueOpen(false);
    setMobileMenuOpen(false);
    setMobileCatalogueOpen(false);
  }, [pathname]);

  // Handle clicking/tapping outside the desktop/tablet dropdown
  useEffect(() => {
    if (!desktopCatalogueOpen) return;

    const handleOutsideClick = (event: MouseEvent | TouchEvent) => {
      if (
        catalogueDropdownRef.current &&
        !catalogueDropdownRef.current.contains(event.target as Node)
      ) {
        setDesktopCatalogueOpen(false);
      }
    };

    // Delay attaching so the opening touch/click does not immediately close it
  ...
  const toggleDesktopCatalogue = (e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setDesktopCatalogueOpen((prev) => !prev);
    setTimeout(() => {
  ...
  const toggleMobileCatalogue = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    setMobileCatalogueOpen((prev) => !prev);
  };
    ...
    <header className="fixed top-3 sm:top-5 left-0 right-0 z-50 px-3 sm:px-6 pointer-events-none flex justify-center">
      ...
      >
        <div className="relative flex items-center justify-between">
          ...
          <nav className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-6 lg:gap-8 z-30">
            <Link
              href="/"
              className={`text-xs font-semibold uppercase tracking-wider transition-all duration-300 font-sans py-1.5 px-2 rounded-lg relative ${
                pathname === '/' 
            ...
            >
              Home
              {pathname === '/' && (
                <span className="absolute bottom-0 left-2 right-2 h-[2px] bg-gradient-to-r from-[#C88B56] to-[#E4B58A] rounded-full" />
              )}
            </Link>

            {/* Catalogue Dropdown (works on tap/click for iPad & tablet, and hover for desktop mouse) */}
            <div 
            ...
            >
              <button
                ...
                onClick={toggleDesktopCatalogue}
                aria-expanded={desktopCatalogueOpen}
                aria-haspopup="true"
                className={`flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider transition-all duration-300 font-sans py-1.5 px-2 rounded-lg cursor-pointer select-none active:scale-95 ${
                  isCatalogueActive || desktopCatalogueOpen
                    ? 'text-[#E4B58A] font-bold'
                    : 'text-stone-300 hover:text-white'
                }`}
              >
                ...
                <ChevronDown 
                  className={`w-3.5 h-3.5 transition-transform duration-300 ${
                    desktopCatalogueOpen ? 'rotate-180 text-[#E4B58A]' : 'text-stone-400'
                  }`} 
                />
              </button>

              <span
                className={`absolute left-2 -bottom-[1px] h-[2px] bg-gradient-to-r from-[#C88B56] to-[#E4B58A] rounded-full transition-all duration-300 ${
                  isCatalogueActive || desktopCatalogueOpen ? 'right-2' : 'left-1/2 right-1/2'
                }`}
              />
              ...
              >
                <div className="w-64 bg-[#101C15]/98 backdrop-blur-2xl border border-white/20 rounded-2xl p-2 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.85)]">
        ...
        <div
          ...
          style={{
            maxHeight: mobileMenuOpen ? '750px' : '0px',
            opacity: mobileMenuOpen ? 1 : 0,
          }}
        >
          <div className="mt-3.5 pt-3.5 pb-2 border-t border-white/10 text-white space-y-3 max-h-[80vh] overflow-y-auto pr-0.5">
            {/* Primary Nav Links with Catalogue Dropdown Toggle */}
            <div className="grid grid-cols-3 gap-2">
              <Link
                href="/"
                onClick={() => setMobileMenuOpen(false)}
                className={`flex items-center justify-center px-3 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 text-center ${
                  pathname === '/'
              ...
              {/* Mobile Catalogue Dropdown Button */}
              <button
                type="button"
                onClick={() => setMobileCatalogueOpen((prev) => !prev)}
                className={`flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 text-center cursor-pointer ${
                  mobileCatalogueOpen || isCatalogueActive
                    ? 'bg-[#C88B56]/25 text-[#E4B58A] border border-[#C88B56]/40'
                    : 'bg-white/5 text-stone-300 hover:bg-white/10 hover:text-white border border-white/5'
                }`}
              >
                ...
                <ChevronDown
                  className={`w-3.5 h-3.5 transition-transform duration-300 ${
                    mobileCatalogueOpen ? 'rotate-180 text-[#E4B58A]' : 'text-stone-400'
                  }`}
                />
              </button>
            ...
            {/* Mobile Catalogue Dropdown Accordion */}
            <div
            ...
            >
              <div className="bg-white/5 rounded-2xl p-2.5 border border-[#C88B56]/30 space-y-1.5 shadow-inner">
                <div className="px-2 pt-1 pb-1 flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-[#E4B58A]">
                  ...
                  <span className="text-[9px] text-stone-400 font-sans normal-case tracking-normal">Select a category</span>
                </div>
